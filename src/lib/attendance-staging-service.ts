...
function mapPayloadToLocalRow(
  ...
  return {
    id: `local-staging-${Date.now()}-${index}`,
...
export async function insertStagingRows(
  ...
  if (error) {
    if (isAttendanceSchemaError(error.message)) {
      throw new Error("attendance_staging missing — run migration 012");
    }
  ...
  for (const payload of payloads) {
    ...
    if (existing) {
      ...
      await insertStagingRows(
        [{ ...payload, is_anomaly: true, anomaly_reason: "Late entry — new row from evening upload" }],
...
export async function transferApprovedToMaster(input: {
  ...
  if (!isSupabaseServerConfigured()) return { transferred: rows.length };
